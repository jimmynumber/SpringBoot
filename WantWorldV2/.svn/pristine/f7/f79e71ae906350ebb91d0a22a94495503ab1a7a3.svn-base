...
public class LoginController {
	...
	@Autowired
	private IwwLoginService iwwLoginService;
	
	@Autowired
	private ICryptoService cryptoService;
	@ApiOperation(value = "登录", nickname="login", notes = "")
	@PostMapping("/login") 
	public WResult<?> login(@RequestBody LoginVo loginVo) { 
...
//		}
		...
		if (userId != null && password != null) {
			...
			return WResultUtil.ok(map); 
			
		} else {
			return WResultUtil.paramError("用户名和密码不能为空！");
		}
			
	}
	
	@ApiOperation(value = "企业微信登录", nickname="iwwlogin", notes = "")
	@PostMapping("/iwwLogin") 
	public WResult<?> iwwlogin(@RequestBody IwwLoginDto iwwloginDto) {
		return iwwLoginService.login(iwwloginDto);
	}
	@ApiOperation(value = "登出", nickname="logout", notes = "")
