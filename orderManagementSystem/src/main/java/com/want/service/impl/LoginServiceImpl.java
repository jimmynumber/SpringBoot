...
//		nowTime= df.format(date);
	...
	public List<FunctionInfo> functionList(String empId){
		...
		if(functionList!=null&&functionList.size()>0) {
			...
			for(FunctionInfo function:functionList) {
				if(function.getFunctionLevel().equals("1")) {
					FunctionInfo functionJson=new FunctionInfo();
					functionJson.setId(function.getId());
					functionJson.setName(function.getName());
					functionJson.setFunctionLevel(function.getFunctionLevel());
					functionJson.setPath(function.getPath());
					functionJson.setIconUrl(function.getIconUrl());
					functionJson.setParentId(function.getParentId());
	...
	public void recursive(String parentFunctionId,FunctionInfo functionJson) { 
		...
		for(int i=1;i<functionList.size();i++) {
			...
			if(parentFunctionId.equals(function.getParentId())) {
				FunctionInfo childFunction=new FunctionInfo();
				childFunction.setId(function.getId());
				childFunction.setName(function.getName());
				childFunction.setFunctionLevel(function.getFunctionLevel());
				childFunction.setPath(function.getPath());
				childFunction.setIconUrl(function.getIconUrl());
				childFunction.setParentId(function.getParentId());
		...
		functionJson.setSubMenuList(list1);
	}
