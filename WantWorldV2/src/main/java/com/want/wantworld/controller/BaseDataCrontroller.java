...
public class BaseDataCrontroller {
	...
	@PassToken
	@ApiOperation(value = "版本更新", nickname="version", notes = "")
	@PostMapping("/update")
	public WResult isUpate(@RequestBody @Valid BaseDataDto baseDataDto){
		return baseDataService.getBaseData(baseDataDto);
	}
	
	@ApiOperation(value = "接口清单", nickname="interfaces", notes = "")
	@PostMapping("/InterfaceList")
    public WResult getInterfaceList(HttpServletRequest request){
        ...
        logger.info("X-Forwarded-Scheme:"+scheme);
        if(null == scheme) {
        	scheme = "https";
        }
                ...
                Arrays.stream(clazzDeclaredMethods).forEach(
                        c -> {
                            if(c.getName().equals(metName)){
                                ...
                                if(null != annotation){
                                    urlDesc.setKey(annotation.nickname());
